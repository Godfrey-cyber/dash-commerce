...
const ComputingCat = ({ computing }) => {
  const [openCatModal, setOpenCatModal] = useState(false)
  const [products, setProducts] = useState(computing)
  ...
  const path = router.query.computing
...
export async function getServerSideProps(context) {
     const category = context.query.computing
    console.log(category)
    const capitalize = s => s && s[0].toUpperCase() + s.slice(1)
    const query = capitalize(category)
    const computing = await axios(`http://localhost:8000/api/product/fetchAll?subcategory=${query}`).then((res) => res.data)
