...
const SmartPhones = () => {
  ...
  return (
      ...
      <section className="grid grid-cols-12 rounded-sm gap-x-4 w-4/5 mx-auto my-6">
        <div className="col-span-3 flex-col space-y-4 rounded-sm h-fit  pb-4 bg-white">
          ...
          <span className="grid grid-cols-12 gap-x-6 gap-y-4 px-2">
              {products.map(item => (
                <div onClick={() => router.push(`/product/${item.id}`)} title={item.name} key={item.id} className="col-span-3 flex flex-col space-y-1 bg-white rounded-sm max-h-fit transition delay-200 hover:shadow-md shadow-gray-500 cursor-pointer relative p-2">
