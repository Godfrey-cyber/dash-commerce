...
const Appliances = ({ appliance }) => {
  ...
  const [products, setProducts] = useState(appliance)
      ...
      <section className="product_section">
        <div className="col-span-3 flex-col space-y-4 rounded-sm h-fit  pb-4 bg-white">
          ...
          <span className="grid grid-cols-12 gap-x-6 gap-y-4 px-2">
              {products?.map(item => (
                <div onClick={() => router.push(`/product/${item._id}`)} title={item.title} key={item.id} className="col-span-3 flex flex-col space-y-1 bg-white rounded-sm max-h-fit transition delay-200 hover:shadow-md shadow-gray-500 cursor-pointer relative p-2">
...
export async function getServerSideProps({params}) {
    // const { appliances } = params
    console.log(params)
    const appliance = await axios("http://localhost:8000/api/product/fetchAll?category=Appliances").then((res) => res.data)
    return {
        props: {
